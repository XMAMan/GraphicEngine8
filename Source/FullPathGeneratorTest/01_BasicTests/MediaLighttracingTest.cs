...
{
    [TestClass]
    public class MediaLighttracingTest //Tests für 1,2,3,4,5
    {
        ...
        public void A_PathLengthOK_PathPdfA_PathContribution() //Test 1,2
        {
            ...
            var maxError = SinglePathCheck.ComparePathContributionWithGeometrySum(method, testSzene, 10000, 3.0e-5f, 1.3184E-11f);
        }
        ...
        public void B_PathPdfAMatchWithFunctionAndHistogramPdfA() //Test 3,4
        {
            ...
            var method = new PathSamplerFactory(testSzene).Create(SamplerEnum.Lighttracing);
            var result = PathPdfAHistogram.ComparePathPdfAWithFunctionPdfAAndHistogram(method, testSzene, testSzene.SampleCountForPathPdfACheck * 2);
            result.Image.Save(WorkingDirectory + "MediaLighttracing.bmp");
            Assert.IsTrue(result.MaxErrorUptoGivenPathLength(3) < 10, "Error=" + result.MaxErrorUptoGivenPathLength(3));
        }
