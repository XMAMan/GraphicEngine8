...
namespace GraphicPipelineOpenGLv3_0
{
    public class GraphicPipelineOpenGLv3_0 : IGraphicPipeline
    {
        ...
        public void DrawSprite(int textureId, int xCount, int yCount, int xBild, int yBild, float x, float y, float width, float height, float texBorder, Color colorFactor)
        {
            ...
            GL.BindTexture(TextureTarget.Texture2D, textureId);
            GL.Begin(PrimitiveType.Quads);
            GL.TexCoord2(xBild * xf + texBorder, yBild * yf + texBorder); GL.Vertex3(x, y, this.ZValue2D);
            GL.TexCoord2((xBild + 1) * xf - texBorder, yBild * yf + texBorder); GL.Vertex3(x + width, y, this.ZValue2D);
